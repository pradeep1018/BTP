...
if __name__=='__main__':
    ...
    for n_run in trange(n_runs):
        ...
        train_auc_df.loc['train auc(crossentropy loss) in run {}'.format(n_run)] = train_auc_cel
        ...
        val_auc_df.loc['val auc(crossentropy loss) in run {}'.format(n_run)] = val_auc_cel
        #val_auc_df.loc['val auc(custom loss)'] = val_auc_cl
        val_auc_df.to_csv('results/chexpert_val_auc.csv')

    print('Mean of Training AUC across all runs')
    print(train_auc_df.mean())
    print('Std of Training AUC across all runs')
    print(train_auc_df.std())

    print('Mean of Validation AUC across all runs')
    print(val_auc_df.mean())
    print('Std of Validation AUC across all runs')
    print(val_auc_df.std())